            ...
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="forms-sample" method="POST" action="{{ route('home.save.fifth.step') }}" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
                                ...
                                @if (is_object($edit_details) && !empty($edit_details->bg_img))
                                    <img src="{{ asset('storage/themes/call_to_action') . '/' . $edit_details->bg_img }}" style="width: 100px;height:100px;">
                                @endif
                            </div>

                            <div class="form-group">
                                <label for="m_title" class="form-label">Main Title</label>
                                <input type="text" name="m_title" class="form-control" placeholder="Call To Action" value="{{ $edit_details->main_title ?? '' }}">
                            </div>
                                ...
                                @if (!empty($details))
                                    @foreach ($details as $detail)
                                        <tr>
                                            ...
                                            <td>
                                                @if (isset($detail) && $detail->bg_img != '')
                                                    <img src="{{ asset('storage/themes/call_to_action') . '/' . $detail->bg_img }}" alt="" style="width: 100px;height:100px;">
                                                @endif
                                            </td>
                                            <td>{{ $detail->desc ?? '' }}</td>
                                            <td>{{ $detail->url ?? '' }}</td>
                                            ...
                                            <td>
                                                <a href="{{ route('home.call.to.action.edit', ['id' => $detail->id]) }}" class="btn btn-dark btn-icon-text">Edit
                                                    <i class="mdi mdi-file-check btn-icon-append"></i>
                                                </a>
                                                <a href="{{ route('home.call.to.action.delete', ['id' => $detail->id]) }}" class="btn btn-danger">Delete
